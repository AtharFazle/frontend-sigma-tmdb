...
import { useGetAnalyticsByAdult } from '@/hooks/useAnalytics';

const {
  data,
  isLoading,
} = useGetAnalyticsByAdult();
</script>

<template>
  <Card class="h-full">
    <CardHeader class="w-full flex flex-row justify-between items-center">
      <CardTitle>Adult Stats</CardTitle>
    ...
    <CardDescription class="p-3">
      <div class="flex flex-row gap-3">
        <p class="text-sm">{{ data?.data?.labels[0] }} : {{ data?.data?.series[0] }}</p>
        <p class="text-sm">{{ data?.data?.labels[1] }} : {{ data?.data?.series[1] }}</p>
      </div>
    </CardDescription>
  </Card>
</template>